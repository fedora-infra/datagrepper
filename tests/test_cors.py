...
class TestCORS(unittest.TestCase):
    ...
    def test_cors(self, grep):
        ...
        self.assertEqual(
            resp.headers["Access-Control-Allow-Origin"], "http://cors.example.com"
        )
    ...
    @patch.dict(
        datagrepper.app.app.config,
        CORS_DOMAINS=[
            re.compile(r"http://safe\.site"),
    ...
    def test_cors_list(self, grep):
        ...
        self.assertTrue("Access-Control-Allow-Origin" in resp.headers)
        self.assertEqual(
            resp.headers["Access-Control-Allow-Origin"], "http://safe.site"
        )
        resp = self.client.get("/raw", headers={"Origin": "http://really.honest"})
        self.assertEqual(resp.status_code, 200)
        self.assertTrue("Access-Control-Allow-Origin" in resp.headers)
        self.assertEqual(
            resp.headers["Access-Control-Allow-Origin"], "http://really.honest"
        )
    ...
    def test_preflight_cors(self, grep):
        resp = self.client.options(
            "/raw", headers={"Origin": "http://preflight.example.com"}
        )
        self.assertEqual(resp.status_code, 200)
        self.assertTrue("Access-Control-Allow-Origin" in resp.headers)
        self.assertEqual(
            resp.headers["Access-Control-Allow-Origin"], "http://preflight.example.com"
        )
    ...
    @patch.dict(
        datagrepper.app.app.config,
        CORS_HEADERS=[re.compile(r"Content-.*"), re.compile(r"X-Other-.*")],
    )
    @patch("datagrepper.app.dm.Message.grep", return_value=(0, 0, []))
